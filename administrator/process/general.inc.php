...
function curCash(){ //FUNCTION: curCash USAGE: To return the current or system initial cash
	...
	$sql_transactions	=	"SELECT c_balance FROM transactions ORDER BY id DESC LIMIT 0,1"; //Retrieve system current cash
	...
	if($results	= $driver->perform_request($sql)):
		$num	=	mysql_num_rows($results);
			if($num>0):
				...
				echo '<div id="recover_s_container"><form id="recover" method="post" action="process/process.php?form=rec">'.$unrecovered.'</form></div>';
			else:
