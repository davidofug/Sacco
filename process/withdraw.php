...
$account	= $driver->clean($_POST['account'],1,1); //Sanitize the selected account type
$client	 	= $driver->clean($_POST['clientaccount'],1,1); //Sanitize the user enetered amount
$amount		= $driver->clean($_POST['amount'],1,1); //Sanitize the selected account type
$accountant	= $driver->clean($_SESSION['name'],1,1);//The system user processing the transaction
if(empty($account) or empty($amount)):
	echo '<p class="error">Please enter complete fields as required! <a href="<?php echo $_SERVER['URL']; ?>/starter.php?page=withdraw.php">Back</a></p>';
	exit();
