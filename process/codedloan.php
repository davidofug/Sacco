...
$duration = $driver->clean($_POST['duration'],1,1);//Sanitize duration
$intrate  = $driver->clean($_POST['interest'],1,1);//Sanitize interest
$curcash  = (curCash() - $principal); //The current cash after removing the principal from the previous balance
$due = round($principal * pow(1 + ($intrate/100),$duration),0);//Amount to pay after the loan has been issued
$interest = $due - $principal;
$repay=mktime(0,0,0,date("m"),date("d")+(loanExpire()*$duration),date("Y")); //Make date when client is supposed to pay back
...
$sql="SELECT * FROM client_transactions WHERE nature='disb' AND balance>'0' AND disbtype='2' AND acnumber='$account'";
$sql2="SELECT pay_date FROM client_transactions WHERE nature='disb' AND balance>'0' AND disbtype='2' AND acnumber='$account' ORDER BY pay_date DESC LIMIT 1";
